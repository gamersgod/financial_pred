...
def get_feat(start, w_size, have_label=True):
    ...
    fund_corr = cosine_similarity(fund_tmp)[decare_iindex]
    d1["fund_cosin"] = fund_corr

    fund_corr_7 = cosine_similarity(fund_tmp.iloc[:, -7:])[decare_iindex]
    d1["fund_cosin_7"] = fund_corr_7

    fund_corr_15 = cosine_similarity(fund_tmp.iloc[:, -15:])[decare_iindex]
    d1["fund_cosin_15"] = fund_corr_15

    fund_corr_30 = cosine_similarity(fund_tmp.iloc[:, -30:])[decare_iindex]
    d1["fund_cosin_30"] = fund_corr_30

    bench_corr = cosine_similarity(bench_tmp)[decare_iindex]
    d1["bench_cosin"] = bench_corr

    bench_corr_7 = cosine_similarity(bench_tmp.iloc[:, -7:])[decare_iindex]
    d1["bench_cosin_7"] = bench_corr_7

    bench_corr_15 = cosine_similarity(bench_tmp.iloc[:, -15:])[decare_iindex]
    d1["bench_cosin_15"] = bench_corr_15

    bench_corr_30 = cosine_similarity(bench_tmp.iloc[:, -30:])[decare_iindex]
    d1["bench_cosin_30"] = bench_corr_30

    fund_diff_corr = cosine_similarity(fund_diff)[decare_iindex]
    d1["fund_diff_cosin"] = fund_diff_corr

    fund_diff_corr_7 = cosine_similarity(fund_diff.iloc[:, -7:])[decare_iindex]
    d1["fund_diff_cosin_7"] = fund_diff_corr_7

    fund_diff_corr_15 = cosine_similarity(fund_diff.iloc[:, -15:])[decare_iindex]
    d1["fund_diff_cosin_15"] = fund_diff_corr_15

    fund_diff_corr_30 = cosine_similarity(fund_diff.iloc[:, -30:])[decare_iindex]
    d1["fund_diff_cosin_30"] = fund_diff_corr_30

    bench_diff_corr = cosine_similarity(bench_diff)[decare_iindex]
    d1["bench_diff_cosin"] = bench_diff_corr

    bench_diff_corr_7 = cosine_similarity(bench_diff.iloc[:, -7:])[decare_iindex]
    d1["bench_diff_cosin_7"] = bench_diff_corr_7

    bench_diff_corr_15 = cosine_similarity(bench_diff.iloc[:, -15:])[decare_iindex]
    d1["bench_diff_cosin_15"] = bench_diff_corr_15

    bench_diff_corr_30 = cosine_similarity(bench_diff.iloc[:, -30:])[decare_iindex]
...
if __name__ == '__main__':
    ...
    delta = 30 # 步长
    data_list = []
    for i in range(21, 481, delta):
        data = get_feat(i, w_size)
        data_list.append(data)

    train_data = pd.concat(data_list, axis=0)
    print(train_data.shape)
    train_data.to_csv("new_data/train_data21_481_30.csv", index=None)

    test_data = get_feat(541, w_size, have_label=False)
    test_data.to_csv("new_data/test_data.csv", index=None)